...
def relative_coordinate_y(f:float,B: float,v:list[float],h:float):
    ...
    '''
    a = np.rad2deg(np.arctan(v/f))
    y = np.tan(np.deg2rad(90-(B-a)))*h
    return y
...
def relative_coordinates(x_center: float, y_center:float, f:float,h:float,B:float, det: list[list[float]]):
    '''
...
def absolute_coordinates(r: float, x: list[float],y: list[float], la:float, lo: float):
    '''
...
# gps_latitude': 47.671755, 'gps_latitude_ref': 'N', 'gps_longitude': 9.269907, 'gps_longitude_ref': 'E', 
# 'altitude': 11.299448948491314, 'gimbal_pitch': 45.4, 'compass_heading': 319.3, 
# 'gimbal_heading': 322.4, 'speed': 2.3429371569065713, 'xspeed': 1.7999517210549845, 'yspeed': -1.4999597675458203, 'zspeed': 0.0